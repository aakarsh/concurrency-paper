...
ordering symantics of following will not be affected.
...
prevent it from merging separate loads from 'a' and seperate stores to
'b'.

TODO leaving this for now its hair raisingly complex shit the
programmer is caught between hot shot compiler writers and hot shot
cpu designers
...
the \lstinline{barrier()} is a genetar barrier. The
...
\item Provide cache coherence for accesses from multiple CPUs to
  single variable. Prevent compiler for reording loads and stores to
  same variable.
